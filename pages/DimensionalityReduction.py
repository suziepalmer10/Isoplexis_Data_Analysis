...
def pca_func(n, method, plot_type, cytokines, df, color_discrete_map):
    try:
        if n is None:
            ...
            if plot_type == "2D":
                fig = px.scatter(
                    df2, x="PCA 1", y="PCA 2", color="Treatment Conditions", color_discrete_map=color_discrete_map)
            else:
                fig = px.scatter_3d(df2, x="PCA 1", y="PCA 2", z="PCA 3",
                                    color="Treatment Conditions", color_discrete_map=color_discrete_map)
    ...
    try:
        if n is None:
            ...
            tsne = TSNE(n_components=3, perplexity=perplexity,
                        n_iter=iterations).fit_transform(principalComponents)
            df = pd.DataFrame(tsne)
            df2 = df.rename({0: "TSNE 1", 1: "TSNE 2", 2: "TSNE 3"}, axis=1)
            df2['Treatment Conditions'] = Y
            if plot_type == "2D":
                fig = px.scatter(df2, x="TSNE 1", y="TSNE 2",
                                 color="Treatment Conditions", color_discrete_map=color_discrete_map)
            else:
                fig = px.scatter_3d(df2, x="TSNE 1", y="TSNE 2", z="TSNE 3",
                                    color="Treatment Conditions", color_discrete_map=color_discrete_map)
