...
def all_prop_test(n, cytokine, match1, match2, df):
    if n is None:
        ...
        if (match1 is None) or (match2 is None):
            ...
            df = pd.DataFrame(df)
            comp_1 = df.loc[df['Treatment Conditions']
                            == match1, cytokine].to_list()
            comp_2 = df.loc[df['Treatment Conditions']
                            == match2, cytokine].to_list()
    ...
    try:
        if n is None:
            ...
            bar_ALL = px.bar(df_edit, x="Cytokines", color="Treatment Conditions",
                             ...
                             height=500)
            bar_ALL.update_layout(
                title_text='Percent Cytokine Secreting', title_x=0.5)
...
def update_bar_chart(n, cytokine, df, color_discrete_map):
    if n is None:
        ...
        fig.update_layout(title_text=cytokine +
                          " Percent Cytokines Secreting", title_x=0.5)
